...
import { SITU, PARTS, ranked, reasons, plateName, styleName, loadPrefs, loadRecent, defaultSitu, type Ctx, type Entry, type Situ } from '@/lib/outfits'
import { colorKeyOf, colorKeysOf, stashPick } from '@/lib/pickPayload'
import { trackEvent } from '@/lib/analytics'

// ═══════════════════════════════════════════════════════
// 조합 목록 — 한 가지 니즈로 거른 후보들 (홈의 "레이어드 조합 추천받기" 등)
// layered: 겹쳐 입는 조합만 · outer: 아우터가 있는 조합만. 누르면 그 조합으로 색 고르기.
// ═══════════════════════════════════════════════════════
...
export default function PickList() {
  ...
  const kind = rawKind && KINDS[rawKind] ? rawKind : 'layered'
  ...
  const list = useMemo(() => {
    ...
    for (const e of ranked(ctx).filter(KINDS[kind])) {
      const sig = PARTS.map(k => e.p[k] || '').join('|'); if (seen.has(sig)) continue
  ...
  const pick = (e: Entry, i: number) => {
    ...
    trackEvent('picks_pick', { kind, id: e.c.id, rank: i + 1, situ, temp })
    navigate('/home/build')
  }
  ...
  return (
    <div className="animate-screen-fade px-5 pt-3 pb-10">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="font-display text-[20px] font-bold tracking-tight text-warm-900 dark:text-warm-100 flex-1">{t(`picks.${kind}.title`)}</h1>
      ...
      <p className="text-[12.5px] text-warm-600 dark:text-warm-400 mb-2">{t(`picks.${kind}.sub`)}</p>
