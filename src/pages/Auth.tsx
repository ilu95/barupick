...
function Login() {
  ...
  const handleSocial = async (provider: 'kakao' | 'google' | 'apple') => {
    try {
      ...
      const msg = e.message || ''; if (msg.includes('not enabled') || msg.includes('Unsupported provider')) { setError(t(`auth.${provider}Login`) + ' ' + t('auth.loginFailed')) } else { setError(t('auth.loginFailed') + ': ' + msg) }
    }
  }
