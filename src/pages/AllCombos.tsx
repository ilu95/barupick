      ...
      <div className="animate-screen-fade px-5 pt-2 pb-10">
        ...
        <div className="bg-warm-100 dark:bg-warm-800 border border-warm-300 dark:border-warm-600 rounded-2xl p-5 mb-5">
          ...
          {['top', 'bottom', 'shoes', 'outer', 'middleware'].map(cat => {
            ...
            const label = t('categories:names.' + cat)
    ...
    <div className="animate-screen-fade px-5 pt-2 pb-10">
      ...
      {weatherComment && (
        <div className="text-sm text-warm-500 dark:text-warm-400 mb-4">
          {weatherComment.emoji} {weatherComment.text} <span className="text-[10px] text-warm-400 dark:text-warm-500">{t('allCombos.reference')}</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <select
          ...
          className="flex-1 py-2 px-3 bg-white dark:bg-warm-800 border border-warm-300 dark:border-warm-600 rounded-2xl text-[12px] font-medium text-warm-700 dark:text-warm-200 appearance-none"
        >
      ...
      <div className="flex flex-col gap-3">
        {filtered.slice(0, showCount).map((combo, idx) => {
          ...
          // 별점
          const scene = sceneFromColors(combo.outfit)

          // 별점
          const stars = combo.score >= 90 ? 5 : combo.score >= 80 ? 4 : combo.score >= 70 ? 3 : combo.score >= 55 ? 2 : 1

          return (
            <div key={key} className="bg-white dark:bg-warm-800 border border-warm-300 dark:border-warm-600 rounded-2xl overflow-hidden shadow-warm-sm">
              {/* 상단: 순위 + 점수 */}
              <div className="flex items-center justify-between px-4 pt-3 pb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-warm-400 dark:text-warm-500">{'★'.repeat(stars)}{'☆'.repeat(5 - stars)}</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-4 px-4 pb-2">
                <div className="bg-warm-100 dark:bg-warm-700 rounded-2xl p-2 flex-shrink-0 flex items-center justify-center">
                  <CharacterCanvas items={scene.items} body={scene.body} width={72} />
                </div>
                  ...
                  {Object.entries(combo.outfit).filter(([_, v]) => v).map(([part, colorKey]) => {
                    ...
                    return (
                      <div key={part} className="flex items-center gap-1.5 text-[11px]">
                        <span className="w-3.5 h-3.5 rounded-full border border-warm-400 dark:border-warm-600 flex-shrink-0" style={{ background: c.hex }} />
                  ...
                  {combo.improvements.map((imp, i) => {
                    ...
                    return (
                      <div key={i} className="text-[10px] text-warm-600 dark:text-warm-400 flex items-center gap-1">
                        ...
                        {newC && <span className="w-2.5 h-2.5 rounded-full inline-block border border-warm-400 dark:border-warm-600" style={{ background: newC.hex }} />}
                      </div>
                    )
                  })}
                </div>
              ...
              {/* CTA: 기록 버튼 */}
              <div className="border-t border-warm-300 dark:border-warm-600 px-3 py-2.5 flex gap-2">
                {isJustRecorded ? (
                  <div className="flex-1 py-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-2xl text-center text-[11px] font-semibold text-green-600 dark:text-green-400 flex items-center justify-center gap-1">
                    <Check size={14} /> {t('ootdRecord.saveComplete')}
                  </div>
                ) : isDup ? (
                  <div className="flex-1 py-2 bg-warm-100 dark:bg-warm-700 rounded-2xl text-center text-[11px] text-warm-500 dark:text-warm-400">
                  ...
                  <>
                    <button
                      ...
                      className="flex-1 py-2 bg-terra-500 text-white rounded-2xl text-[11px] font-semibold flex items-center justify-center gap-1 active:scale-[0.97] transition-all"
                    >
                      <Check size={13} /> {t('allCombos.woreToday')}
                    </button>
                    <button
                      onClick={() => { setDatePickerFor(key); setDatePickerValue('') }}
                      className="py-2 px-3 bg-warm-100 dark:bg-warm-700 border border-warm-300 dark:border-warm-600 rounded-2xl text-[11px] font-medium text-warm-600 dark:text-warm-400 flex items-center gap-1 active:scale-[0.97] transition-all"
                    >
              ...
              {datePickerFor === key && (
                <div className="px-3 pb-3 flex items-center gap-2 animate-screen-fade">
                  <input
                    ...
                    max={todayStr()}
                    className="flex-1 py-2 px-3 bg-warm-50 dark:bg-warm-700 border border-warm-300 dark:border-warm-600 rounded-2xl text-[12px] text-warm-800 dark:text-warm-200"
                  />
                  <button
                    onClick={() => handleRecordOnDate(combo.outfit)}
                    disabled={!datePickerValue}
                    className="py-2 px-4 bg-terra-500 text-white rounded-2xl text-[11px] font-semibold disabled:opacity-40 active:scale-[0.97] transition-all"
                  >
                    {t('allCombos.record')}
                  </button>
                  <button
                    onClick={() => setDatePickerFor(null)}
                    className="py-2 px-2 text-warm-500 dark:text-warm-400 text-[11px]"
                  >
                    {t('common.cancel')}
                  </button>
                </div>
      ...
      {combos.length > 0 && combos.length <= 5 && (
        <div className="mt-5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl p-4">
          ...
          <button
            ...
            className="w-full py-2.5 bg-white dark:bg-warm-800 border border-amber-300 dark:border-amber-700 rounded-2xl text-[12px] font-semibold text-amber-700 dark:text-amber-300 flex items-center justify-center gap-1.5 active:scale-[0.98] transition-all"
          >
